...
pub(crate) fn operate(mode: ChatMode, llm: Llama<f32>, tokenizer: Tokenizer) -> Result<()> {
    ...
    if let Ok(_) = stdin().read_line(&mut input) {
        ...
        let output_ids = llm.generate(input_ids, mode.model_args.max_length as usize, mode.model_args.top_p, mode.model_args.top_k, mode.model_args.temperature);
        ...
        if mode.model_args.stream {
            ...
            eprint!("{:?}", output.decode(output_ids.collect::<Vec<u32>>().as_slice()))
        }
        eprint!("\nUser: ");
    }
...
mod tests {
    ...
    fn test_jinja2_template() {
        ...
        context.insert("messages", &messages);
        context.insert("add_generation_prompt", &true);
        let input = tera.render("chatml.jinja2", &context).unwrap().trim().to_string();
        assert_eq!(input, "<|im_start|>system\nHello, how can I help you?<|im_end|>\n<|im_start|>assistant");
    }
}